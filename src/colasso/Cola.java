...
public class Cola {
    ...
    public Nodo getNodo(String nombre){
        Nodo inicio=getProcesador();
        while(inicio.getSiguiente()!=procesador){
            if(inicio.getSiguiente().getNombre().equals(nombre)){
    ...
    public void agregar(Nodo n, int nombre, int tiempoLlegada, int tiempoRafaga) {
        if (n.getSiguiente() == procesador) {
            ...
            if (tiempoRafaga < n.getSiguiente().getTiempoRafaga()) {
                Nodo nuevoNodo = new Nodo(String.valueOf(nombre), tiempoLlegada, tiempoRafaga, getProcesador());
                nuevoNodo.setSiguiente(n.getSiguiente());
                n.setSiguiente(nuevoNodo);
            } else {
    ...
    public String[] guardarDatos(Nodo n) {
        ...
        datos[2] = String.valueOf(n.getTiempoEjecucion());
